/*
 ...
 dziedziczenia. Klasy te opisuja elementarne Figuray. Kazdy Figura
...
int main() {
	...
	Figura::wypisz(k); // Kwadrat o boku: 4
	Figura::wypisz(t); // Trojkat o bokach: 3 2 4
	Figura::wypisz(ko);// Kolo o promieniu: 3

	Figura* b = new Kwadrat(7);
	Figura::wypisz(*b);
	delete b;
	...
	const Figura* wszystkie[] = { &k, &k, &ko, &t, &ko };
	const int rozmiar = sizeof(wszystkie)/sizeof(Figura*);
	double pole_calkowite = 0.0;
	for ( int idx = 0; idx < rozmiar; ++idx ) {
		const Figura* k = wszystkie[idx];
		k->wypisz(std::cout);
